...
def load_app_config():
    ...
    if not os.path.exists(CONFIG_FILE_PATH):
        ...
        logger.warning(f"未找到配置文件或 JWT_SECRET_KEY。已生成临时 JWT_SECRET_KEY: {JWT_SECRET_KEY}")
        logger.warning("请将其添加到您的 config.ini 文件中的 [jwt] 部分，例如: secret_key = YOUR_GENERATED_KEY")
        return
    ...
    try:
        ...
        if 'proxy_auth' in config_parser and 'api_keys' in config_parser['proxy_auth']:
            ...
            if PROXY_API_KEYS:
                logger.info(f"从 '{CONFIG_FILE_PATH}' 加载了 {len(PROXY_API_KEYS)} 个代理 API Key。")
            else:
            ...
            logger.info(f"Proxy API Key Header 设置为: '{PROXY_API_KEY_HEADER}'")
        else:
        ...
        if 'jwt' in config_parser:
            ...
            if not JWT_SECRET_KEY:
                ...
                JWT_SECRET_KEY = secrets.token_urlsafe(32)
                logger.warning(
                    f"在 '{CONFIG_FILE_PATH}' 的 [jwt] 部分未找到 'secret_key'。已生成临时 JWT_SECRET_KEY: {JWT_SECRET_KEY}"
                )
                logger.warning("强烈建议您在 config.ini 中设置一个持久的 secret_key。")

            logger.info(f"JWT 配置已加载: 算法='{JWT_ALGORITHM}', Token有效期={JWT_ACCESS_TOKEN_EXPIRE_MINUTES}分钟。")
        else:
            logger.warning(f"在 '{CONFIG_FILE_PATH}' 中未找到 [jwt] 部分。将使用默认 JWT 设置和生成的密钥。")
            import secrets

            JWT_SECRET_KEY = secrets.token_urlsafe(32)  # Ensure secret key is set
            logger.warning(f"已生成临时 JWT_SECRET_KEY: {JWT_SECRET_KEY}。请添加到 config.ini。")

        # 加载应用设置
        if 'App' in config_parser:
            APP_CONFIG_MAX_RETRIES = config_parser['App'].getint('max_retries', 5)
            if APP_CONFIG_MAX_RETRIES <= 0:
                logger.warning(f"[App] max_retries 配置值 ({APP_CONFIG_MAX_RETRIES}) 无效。将使用默认值 1。")
                APP_CONFIG_MAX_RETRIES = 1  # Ensure at least 1 attempt
            else:
                logger.info(f"[App] max_retries 配置已加载: {APP_CONFIG_MAX_RETRIES}")
        else:
            # 如果未找到 [App] 部分或 max_retries，APP_CONFIG_MAX_RETRIES 将保持其默认值 (5)
            logger.warning(
                f"在 '{CONFIG_FILE_PATH}' 中未找到 [App] 部分或 'max_retries' 键。将使用默认 max_retries: {APP_CONFIG_MAX_RETRIES}。"
            )
        ...
        if not JWT_SECRET_KEY:  # 即使在解析错误时也确保设置了密钥
            ...
            JWT_SECRET_KEY = secrets.token_urlsafe(32)
            logger.error(f"配置文件解析错误。已生成临时 JWT_SECRET_KEY: {JWT_SECRET_KEY}。请检查 config.ini。")
        ...
        if not JWT_SECRET_KEY:  # 即使在其他错误时也确保设置了密钥
            ...
            JWT_SECRET_KEY = secrets.token_urlsafe(32)
            logger.error(f"未知配置错误。已生成临时 JWT_SECRET_KEY: {JWT_SECRET_KEY}。")
